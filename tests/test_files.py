...
def test_handle_dst_file_2(txt_file, capsys):
    ...
    expected_output = (f"{ERROR}: The destination path provided: {txt_file} is an already existing file. "
...
def test_handle_dst_file_4(fake_path, monkeypatch, capsys):
    ...
    result = files.handle_dst_file(fake_path)
    expected_output = f"{ERROR}: Path doesn't exist and user does not want to create path."
...
def test_check_overwrite_file_3(dir_path, capsys):
    ...
    output = capsys.readouterr().out.strip()
    expected_output = f"{ERROR}: The path provided: {dir_path} is a directory, not a file."
    assert result is False
    assert output == expected_output
...
def test_exe_file_copy_1(txt_file, dst_path, capsys):
    ...
    output = capsys.readouterr().out.strip()
    expected_output = f"{SUCCESS}: File {txt_file.name} was successfully copied to {dst_path}"
